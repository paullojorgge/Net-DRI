...
is($rc->is_success(),1,'message_retrieve is_success');
is_string($R1,$E1.'<command><poll op="req"/><clTRID>ABC-12345</clTRID></command>'.$E2,'message_retrieve build xml');
is($rc->get_data('message',100007,'object_type'),'drp','notification drp object_type');
is($rc->get_data('message',100007,'name'),'drp','notification drp name');
is($rc->get_data('message',100007,'correspondentietaal'),'eng','notification drp correspondentietaal');
is($rc->get_data('message',100007,'deelnemernummer'),123456,'notification drp deelnemernummer');
